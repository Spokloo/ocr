...
{
    float P[9][9] = {{-src[0], -src[1], -1, 0, 0, 0, src[0] * dst[0],
                      src[1] * dst[0], dst[0]},
                     {0, 0, 0, -src[0], -src[1], -1, src[0] * dst[1],
                      src[1] * dst[1], dst[1]},
                     {-src[2], -src[3], -1, 0, 0, 0, src[2] * dst[2],
                      src[3] * dst[2], dst[2]},
                     {0, 0, 0, -src[2], -src[3], -1, src[2] * dst[3],
                      src[3] * dst[3], dst[3]},
                     {-src[4], -src[5], -1, 0, 0, 0, src[4] * dst[4],
                      src[5] * dst[4], dst[4]},
                     {0, 0, 0, -src[4], -src[5], -1, src[4] * dst[5],
                      src[5] * dst[5], dst[5]},
                     {-src[6], -src[7], -1, 0, 0, 0, src[6] * dst[6],
                      src[7] * dst[6], dst[6]},
                     {0, 0, 0, -src[6], -src[7], -1, src[6] * dst[7],
                      src[7] * dst[7], dst[7]},
                     {0, 0, 0, 0, 0, 0, 0, 0, 1}};
...
void correct_perspective(Image *img, int src[8])
{
    ...
    unsigned int top_length =
        sqrt(pow(src[0] - src[2], 2) + pow(src[1] - src[3], 2));
    unsigned int right_length =
        sqrt(pow(src[2] - src[4], 2) + pow(src[3] - src[5], 2));
    unsigned int bottom_length =
        sqrt(pow(src[4] - src[6], 2) + pow(src[5] - src[7], 2));
    unsigned int left_length =
        sqrt(pow(src[6] - src[0], 2) + pow(src[7] - src[1], 2));

    // new image will be a square
    // get max length which represent the length of the sides of our new grid
    int max_length =
        fmax(fmax(top_length, bottom_length), fmax(right_length, left_length));
    int dst[8] = {0, 0, max_length, 0, max_length, max_length, 0, max_length};
    ...
    if (img->matrix == NULL)
        errx(1, "Error during allocation of pixels' matrix in perspective "
                "correction.");
