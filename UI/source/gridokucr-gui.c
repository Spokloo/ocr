...
UI *init_ui(GtkBuilder *builder)
{
    ...
    ui->window =
        GTK_WINDOW(gtk_builder_get_object(builder, "org.gtk.gridokucr"));
    ...
    step1->file_chooser_button = GTK_FILE_CHOOSER_BUTTON(
    ...
    step2->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "select_button_step2"));
    step2->buttons[1] =
        GTK_BUTTON(gtk_builder_get_object(builder, "confirm_button_step2"));
    step2->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step2"));
    step2->labels[0] =
        GTK_LABEL(gtk_builder_get_object(builder, "image_path_step2"));
    ...
    step3->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "previous_step3"));
    step3->buttons[1] =
        GTK_BUTTON(gtk_builder_get_object(builder, "next_step3"));
    step3->buttons[2] =
        GTK_BUTTON(gtk_builder_get_object(builder, "last_step3"));
    step3->buttons[3] =
        GTK_BUTTON(gtk_builder_get_object(builder, "confirm_filters_step3"));
    step3->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step3"));
    step3->labels[0] =
        GTK_LABEL(gtk_builder_get_object(builder, "sub_step_name_step3"));
    ...
    step4->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "confirm_rotation_step4"));
    step4->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step4"));
    step4->scales[0] =
        GTK_WIDGET(gtk_builder_get_object(builder, "spin_step4"));
    step4->scales[1] =
        GTK_WIDGET(gtk_builder_get_object(builder, "scale_step4"));
    ...
    step5->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "grid_detect_step5"));
    step5->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step5"));
    ...
    step6->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "previous_step6"));
    step6->buttons[1] =
        GTK_BUTTON(gtk_builder_get_object(builder, "next_step6"));
    step6->buttons[2] =
        GTK_BUTTON(gtk_builder_get_object(builder, "last_step6"));
    step6->buttons[3] =
        GTK_BUTTON(gtk_builder_get_object(builder, "confirm_detect_step6"));
    step6->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step6"));
    step6->labels[0] =
        GTK_LABEL(gtk_builder_get_object(builder, "sub_step_name_step6"));
    for (int i = 0; i < 4; i++)
    {
    ...
    step8->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "digit_recog_step8"));
    step8->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step8"));
    ...
    step9->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "place_digit_step9"));
    step9->buttons[1] =
        GTK_BUTTON(gtk_builder_get_object(builder, "confirm_recog_step9"));
    step9->draw_areas[0] = GTK_DRAWING_AREA(
        gtk_builder_get_object(builder, "draw_area_det_step9"));
    step9->draw_areas[1] = GTK_DRAWING_AREA(
        gtk_builder_get_object(builder, "draw_area_gen_step9"));
    step9->entries[0] =
        GTK_ENTRY(gtk_builder_get_object(builder, "line_entry_step9"));
    step9->entries[1] =
        GTK_ENTRY(gtk_builder_get_object(builder, "column_entry_step9"));
    step9->entries[2] =
        GTK_ENTRY(gtk_builder_get_object(builder, "digit_entry_step9"));
    ...
    step10->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "launch_solver_step10"));
    step10->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step10"));
    ...
    step11->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "save_solved_grid_step11"));
    step11->draw_areas[0] =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "draw_area_step11"));
    ...
    ui->header->header_bar =
        GTK_HEADER_BAR(gtk_builder_get_object(builder, "header_bar"));
    ui->header->buttons = malloc(sizeof(GtkButton *) * 2);
    ui->header->buttons[0] =
        GTK_BUTTON(gtk_builder_get_object(builder, "reload_button"));
    ui->header->buttons[1] =
        GTK_BUTTON(gtk_builder_get_object(builder, "save_img_button"));
    ui->header->button = GTK_FILE_CHOOSER_BUTTON(
        gtk_builder_get_object(builder, "open_img_button"));
    ...
    ui->progress->buttons = malloc(sizeof(GtkButton *) * 11);
    ui->progress->buttons[1] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button1"));
    ui->progress->buttons[2] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button2"));
    ui->progress->buttons[3] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button3"));
    ui->progress->buttons[4] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button4"));
    ui->progress->buttons[7] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button6"));
    ui->progress->buttons[9] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button7"));
    ui->progress->buttons[10] =
        GTK_BUTTON(gtk_builder_get_object(builder, "step_button8"));
    ...
    for (int i = 0; i < 11; i++)
    {
        if (i != 6)
        {
            ...
            ui->steps[i]->curr_img = 0;
        }
    }

    ui->curr_img = malloc(sizeof(Image));

    ui->dialog = GTK_MESSAGE_DIALOG(gtk_builder_get_object(builder, "dialog"));
...
void connect_signals(UI *ui)
{
    ...
    g_signal_connect(ui->steps[0]->file_chooser_button, "file-set",
                     G_CALLBACK(on_file_set), ui);

    g_signal_connect(ui->steps[1]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step2), ui);
    g_signal_connect(ui->steps[1]->buttons[0], "clicked",
                     G_CALLBACK(cancel_select), ui);
    g_signal_connect(ui->steps[1]->buttons[1], "clicked",
                     G_CALLBACK(confirm_select), ui);

    g_signal_connect(ui->steps[2]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step3), ui);
    g_signal_connect(ui->steps[2]->buttons[0], "clicked",
                     G_CALLBACK(on_previous_step3), ui);
    g_signal_connect(ui->steps[2]->buttons[1], "clicked",
                     G_CALLBACK(on_next_step3), ui);
    g_signal_connect(ui->steps[2]->buttons[2], "clicked",
                     G_CALLBACK(on_skip_step3), ui);
    g_signal_connect(ui->steps[2]->buttons[3], "clicked",
                     G_CALLBACK(confirm_sub_steps2), ui);

    g_signal_connect(ui->steps[3]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step4), ui);
    g_signal_connect(ui->steps[3]->scales[0], "value-changed",
                     G_CALLBACK(on_spin_value_changed), ui);
    g_signal_connect(ui->steps[3]->scales[1], "value-changed",
                     G_CALLBACK(on_scale_value_changed), ui);
    g_signal_connect(ui->steps[3]->buttons[0], "clicked",
                     G_CALLBACK(confirm_rotation), ui);

    g_signal_connect(ui->steps[4]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step5), ui);
    g_signal_connect(ui->steps[4]->buttons[0], "clicked",
                     G_CALLBACK(launch_grid_detect), ui);

    g_signal_connect(ui->steps[5]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step6), ui);
    g_signal_connect(ui->steps[5]->buttons[0], "clicked",
                     G_CALLBACK(on_previous_step6), ui);
    g_signal_connect(ui->steps[5]->buttons[1], "clicked",
                     G_CALLBACK(on_next_step6), ui);
    g_signal_connect(ui->steps[5]->buttons[2], "clicked",
                     G_CALLBACK(on_skip_step6), ui);
    g_signal_connect(ui->steps[5]->buttons[3], "clicked",
                     G_CALLBACK(confirm_sub_steps5), ui);

    g_signal_connect(ui->steps[7]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step8), ui);
    g_signal_connect(ui->steps[7]->buttons[0], "clicked",
                     G_CALLBACK(on_launch_digit_recog), ui);

    g_signal_connect(ui->steps[8]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step9_0), ui);
    g_signal_connect(ui->steps[8]->draw_areas[1], "draw",
                     G_CALLBACK(on_draw_step9_1), ui);
    g_signal_connect(ui->steps[8]->buttons[0], "clicked", G_CALLBACK(on_place),
                     ui);
    g_signal_connect(ui->steps[8]->buttons[1], "clicked",
                     G_CALLBACK(on_confirm_digit), ui);

    g_signal_connect(ui->steps[9]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step10), ui);
    g_signal_connect(ui->steps[9]->buttons[0], "clicked",
                     G_CALLBACK(launch_solver), ui);

    g_signal_connect(ui->steps[10]->draw_areas[0], "draw",
                     G_CALLBACK(on_draw_step11), ui);
    g_signal_connect(ui->steps[10]->buttons[0], "clicked",
                     G_CALLBACK(save_solved_image), ui);

    ///// HEADER BAR /////
    g_signal_connect(ui->header->button, "file-set", G_CALLBACK(on_open_header),
                     ui);
    g_signal_connect(ui->header->buttons[0], "clicked", G_CALLBACK(on_restart),
                     ui);
    g_signal_connect(ui->header->buttons[1], "clicked",
                     G_CALLBACK(on_save_header), ui);

    ///// PROGRESS BAR /////
    g_signal_connect(ui->progress->buttons[1], "clicked",
                     G_CALLBACK(on_load_image), ui);
    g_signal_connect(ui->progress->buttons[2], "clicked",
                     G_CALLBACK(on_pre_proc), ui);
    g_signal_connect(ui->progress->buttons[3], "clicked",
                     G_CALLBACK(on_man_rot), ui);
    g_signal_connect(ui->progress->buttons[4], "clicked",
                     G_CALLBACK(on_grid_detect), ui);
    g_signal_connect(ui->progress->buttons[7], "clicked",
                     G_CALLBACK(on_digit_recog), ui);
    g_signal_connect(ui->progress->buttons[9], "clicked",
                     G_CALLBACK(on_solve_grid), ui);
    g_signal_connect(ui->progress->buttons[10], "clicked",
                     G_CALLBACK(on_save_solved), ui);
}

void set_step(UI *ui, int num)
{
    gtk_stack_set_visible_child(ui->stack,
                                GTK_WIDGET(ui->steps[num]->viewport));
    ...
    if (num == 1 || num == 2 || num == 3 || num == 4 || num == 7 || num == 9 ||
        num == 10)
        gtk_widget_set_sensitive(GTK_WIDGET(ui->progress->buttons[num]), TRUE);
    gtk_progress_bar_set_fraction(ui->progress->progress_bar,
                                  ui->curr_step / 10.0);
}

void display_image(GtkDrawingArea *draw_area, Image *img, UI *ui, int step,
                   int image)
{
...
void copy_hough_param(UI *ui, int from, int to)
{
    ui->steps[5]->hough_param[to]->lines_len =
        ui->steps[5]->hough_param[from]->lines_len;
    ...
    ui->steps[5]->hough_param[to]->real_lines =
        malloc(sizeof(int *) * ui->steps[5]->hough_param[to]->lines_len);
    for (unsigned int i = 0; i < ui->steps[5]->hough_param[from]->lines_len;
         i++)
    {
...
{
    GdkPixbuf *pixbuf =
        gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, img->width, img->height);
                ...
                int img)
{
    ...
    pixbuf = gdk_pixbuf_scale_simple(pixbuf, image_w_res, image_h_res,
                                     GDK_INTERP_BILINEAR);
...
void set_sub_step_label(UI *ui, int step)
{
    ...
    case 1: {
        if (step == 5)
            gtk_label_set_text(ui->steps[5]->labels[0],
    ...
    case 2: {
        if (step == 5)
            ...
            gtk_label_set_text(ui->steps[2]->labels[0],
...
void next_sub_step(GtkButton *button, UI *ui, int step)
{
    ...
    if (step == 2)
    {
        ...
        case 3: {
            ...
            unsigned char filter_size =
                blur.width > blur.height ? blur.width / 300 : blur.height / 300;
        ...
        case 4: {
            ...
            unsigned char filter_size = dilat.width > dilat.height
                                            ? dilat.width / 300
                                            : dilat.height / 300;
        ...
        case 5: {
            ...
            unsigned char filter_size =
                eros.width > eros.height ? eros.width / 300 : eros.height / 300;
        ...
        case 6: {
            ...
            gtk_widget_set_sensitive(GTK_WIDGET(button), FALSE);
            gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[2]->buttons[2]),
                                     FALSE);
            gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[2]->buttons[3]),
                                     TRUE);
            break;
        }
        default: {
    ...
    else
    {
        ...
        case 1: {
            ...
            hough_transform(&hough, &ui->steps[5]->hough_param[0]->lines_len,
                            &ui->steps[5]->hough_param[0]->real_lines);
        ...
        case 2: {
            ...
            int angle =
                auto_rotation(&auto_rot, ui->steps[5]->images[0],
                              &ui->steps[5]->hough_param[1]->lines_len,
                              &ui->steps[5]->hough_param[1]->real_lines,
                              &ui->steps[5]->hough_param[1]->result_imgs);
        ...
        case 3: {
            ...
            squares(&squ, &ui->steps[5]->hough_param[2]->lines_len,
                    &ui->steps[5]->hough_param[2]->real_lines,
                    &ui->steps[5]->hough_param[2]->sq,
                    &ui->steps[5]->hough_param[2]->gs,
                    &ui->steps[5]->hough_param[2]->result_imgs);
        ...
        case 4: {
            ...
            gtk_widget_set_sensitive(GTK_WIDGET(button), FALSE);
            gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[5]->buttons[2]),
                                     FALSE);
            gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[5]->buttons[3]),
                                     TRUE);
            ...
            if (solved_grid[i][j] != 0)
            {
                if (old_grid != NULL)
                {
                    if (old_grid[i][j] != solved_grid[i][j])
                        place_img(&grid_img, &red_color, solved_grid[i][j], x,
                                  y);
                    else
                        place_img(&grid_img, &normal_color, solved_grid[i][j],
                                  x, y);
                }
                else
                    place_img(&grid_img, &normal_color, solved_grid[i][j], x,
                              y);
            }
...
void display_grid_not_solvable(UI *ui)
{
    gtk_dialog_run(GTK_DIALOG(ui->dialog));
    gtk_widget_hide(GTK_WIDGET(ui->dialog));
}

void restart_from(UI *ui, int step)
{
    for (unsigned int i = step; i < 11; i++)
    {
        if (i != 6)
        {
            ui->steps[i]->sub_step = 0;
        ...
        // STEP 3 buttons
        gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[2]->buttons[0]), FALSE);
        ...
        // STEP 6 buttons
        gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[5]->buttons[0]), FALSE);
        gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[5]->buttons[1]), TRUE);
        gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[5]->buttons[2]), TRUE);
        gtk_widget_set_sensitive(GTK_WIDGET(ui->steps[5]->buttons[3]), FALSE);
    }
...
void confirm_select(GtkButton *button, gpointer user_data)
{
    ...
    gtk_button_get_label(button); // prevent unused param
    set_step(ui, 2);
    display_image(ui->steps[2]->draw_areas[0], ui->steps[1]->images[0], ui, 2,
                  0);
    set_sub_step_label(ui, 2);
}
...
void confirm_sub_steps2(GtkButton *button, gpointer user_data)
{
    ...
    set_step(ui, 3);
    copy_image(ui->steps[2]->images[ui->steps[2]->curr_img],
               ui->steps[3]->images[0]);
    copy_image(ui->steps[2]->images[ui->steps[2]->curr_img],
               ui->steps[3]->images[1]);
...
void launch_solver(GtkButton *button, gpointer user_data)
{
    ...
    if (ui->steps[10]->grid == NULL)
        display_grid_not_solvable(ui);
    else
    {
...
void on_open_header(GtkFileChooserButton *button, gpointer user_data)
{
    ...
    display_image(ui->steps[ui->curr_step]->draw_areas[0], &img, ui,
                  ui->curr_step, 0);
