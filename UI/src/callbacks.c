...
void on_save_button_clicked(GtkButton *button, gpointer user_data)
{
    ...
    if (var->filename_solv != NULL)
    {
        ...
        dialog = gtk_file_chooser_dialog_new(
            "Save file", GTK_WINDOW(var->parent_window), action, "Cancel",
        ...
        if (res == GTK_RESPONSE_ACCEPT)
        {
            ...
            GdkPixbuf *pixbuf =
                gdk_pixbuf_new_from_file(var->filename_solv, &error);
            ...
            else if (!gdk_pixbuf_save(pixbuf, filename, "jpeg", &error,
                                      "quality", "100", NULL))
            {
...
void on_open_button_clicked(GtkButton *button, gpointer user_data)
{
    ...
    dialog = gtk_file_chooser_dialog_new(
        "Open file", GTK_WINDOW(var->parent_window), action, "Cancel",
        GTK_RESPONSE_CANCEL, "Save", GTK_RESPONSE_ACCEPT, NULL);
...
void on_rotate_scale_value_changed(GtkRange *rotate_scale, gpointer user_data)
{
    ...
    if (gtk_image_get_storage_type(GTK_IMAGE(var->former_image)) ==
        GTK_IMAGE_PIXBUF)
    {
