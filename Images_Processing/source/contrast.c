...
void contrast(Image *img, int level)
{
    ...
    Pixel pixel;
    float factor = (259 * ((float)level + 255)) / (255 * (259 - (float)level));
    ...
    for (unsigned int x = 0; x < img->width; x++)
    {
        for (unsigned int y = 0; y < img->height; y++)
        {
            ...
            img->matrix[x][y].r = truncate(factor * ((int)pixel.r - 128) + 128);
            img->matrix[x][y].g = truncate(factor * ((int)pixel.g - 128) + 128);
            img->matrix[x][y].b = truncate(factor * ((int)pixel.b - 128) + 128);
        }
    }
}
